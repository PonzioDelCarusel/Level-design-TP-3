...
local IDLE_STANCE = script:GetCustomProperty("IdleStance") or "zombie_unarmed_idle_relaxed"
...
function PlayAttack()
	if attackIndex == 1 then
		MESH:PlayAnimation("zombie_unarmed_grab", {playbackRate = 0.6})
		attackIndex = 2
	else
		MESH:PlayAnimation("zombie_unarmed_grab", {playbackRate = 1})
		attackIndex = 1
	end
...
function PlayDamaged()
	MESH:PlayAnimation("zombie_unarmed_idle_ready")
end
